...
class MacroRecorder:
    ...
    def on_event(self, event):
        if self.is_recording:
            ...
            elif isinstance(event, mouse.ButtonEvent) or isinstance(event, mouse.MoveEvent):
                self.events.append({'type': 'mouse', 'event': event.event_type, 'x': event.x, 'y': event.y, 'time': event_time})
    ...
    def play(self):
        ...
        for event in self.events:
            time.sleep(max(0, event['time'] - last_event_time))
            ...
            if event['type'] == 'keyboard':
                ...
                if event['event'] == 'move':
                    ...
                    mouse.click(event['event'], x=event['x'], y=event['y'])
